...
from app.account_actions import is_sub2api_auto_upload_enabled, run_sub2api_upload_for_account
...
from app.plus_binding import run_plus_binding_with_browser_session
    ...
    try:
        ...
        if activation_result.success:
            ...
            if is_sub2api_auto_upload_enabled():
                ...
                update_account_status(
                    email,
                    "已上传Sub2Api" if upload_result.uploaded else "Sub2Api上传失败",
                    extra={
                        "sub2apiUploaded": bool(upload_result.uploaded),
                        "sub2apiStatus": "已上传" if upload_result.uploaded else "上传失败",
                        "sub2apiMessage": upload_result.message,
                        "sub2apiUploadedAt": time.strftime("%Y%m%d_%H%M%S") if upload_result.uploaded else "",
                        "oauthTokens": upload_result.tokens,
                        "oauthOutputFile": upload_result.output_file,
                    },
                )
                _report("sub2api_uploaded" if upload_result.uploaded else "sub2api_upload_failed")
            else:
                update_account_status(
                    email,
                    "已激活Plus，未上传Sub2Api",
                ...
                _report("sub2api_auto_upload_disabled")
        else:
            ...
            if activation_result.stage == "config":
                ...
                _report("activation_config_missing")
            elif activation_result.stage == "fetch_token":
                update_account_status(
                    email,
                    "Token获取失败",
                    access_token=activation_result.access_token or None,
